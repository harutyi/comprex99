  ...
  function render(estimate) {
      ...
      <article class="quote" id="quoteSheet">
        ...
        <section class="terms">
          ...
          <ul>
            ...
            <li>おまかせ制作の管理契約は、完成確認・顧客OK後に別途お手続きいただきます。請求開始日はStripeの決済画面でご確認ください。</li>
            <li>おまかせ制作は12ヶ月の管理契約が前提です。買い切り制作では管理契約は必須ではありません。</li>
    ...
    document.querySelector("#exportImage").addEventListener("click", () => {
      ...
      [
        ...
        `おまかせ制作の管理契約は、完成確認・顧客OK後に別途お手続きいただきます。請求開始日はStripeの決済画面でご確認ください。`,
        "おまかせ制作は12ヶ月の管理契約が前提。買い切り制作では不要です。",
    ...
    checkout.addEventListener("click", async () => {
      ...
      const result = await response.json();
      if (!response.ok) {
        message.textContent = result.error || "決済ページへ進めませんでした。";
        return;
      }
